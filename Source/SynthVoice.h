...
#define N 101
#define LENGTH 0.7
#define GAMMA 0.00005
#define STIFFNESS 0.00005
...
public:
    ...
    void renderNextBlock(AudioBuffer<float>& outputBuffer, int startSample, int numSamples) override {
        ...
        float nextState[N];
        std::fill(nextState, nextState + N, 0);

        for (int i = 2; i < N - 2; i++) {
            float t0 = 1.0 / (pow(c, 2) * pow(dt, 2) + GAMMA / (2 * dt));
            float t1 = 1.0 / pow(dx, 2) * (previousState[i - 1] - 2 * currentState[i] + currentState[i + 1]);
            float t2 = 1.0 / (pow(c, 2) * pow(dt, 2)) * (previousState[i] - 2 * currentState[i]);
            float t3 = GAMMA / (2 * dt) * previousState[i];
            float t4 = (pow(STIFFNESS, 2) / pow(dx, 4)) * (currentState[i + 2] - 4 * currentState[i + 1] + 6 * currentState[i] - 4 * currentState[i - 1] + currentState[i - 2]);
        ...
        for (int n = 0; n < 10; n++) {
            for (int i = 0; i < N; i++) {
                sum += currentState[i] * sin(n * double_Pi * (i / 101.0)) / 101.0;
            }
        }
        ...
        std::copy(previousState, previousState + N, currentState);
        std::copy(currentState, currentState + N, nextState);
    }

private:
    double sampleRate = getSampleRate();

    double dx = LENGTH / 100.0;
